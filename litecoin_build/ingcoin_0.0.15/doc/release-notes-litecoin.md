INGcoin Core version *0.15.1* is now available from:

  <https://download.ingcoin.org/ingcoin-0.15.1/>
...
A number of changes to the way INGcoin Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 ings per kB (0.00001 ING). This is to prepare for dropping the minimum transaction fee to 0.1 ings per kB (0.0001 ING) in the next release.
